...
namespace DB {
	...
	//更新
	class Update : public DB {
	public:
		void Customer(int id, std::string name, int isTrue);
		void ExpensesItem(int id,std::string name, double rate,int isTrue);
		void Expenses(int id,std::string date, int item, int money);
		void Unpaid(int id, int money);
		void Sales(int id,std::string date, int customer, int money,std::string tax, std::string person, std::string subject);
	};
}